 ...
 */
class Node {
  constructor (data, prev = null, next = null, list = null) {
    this.data = data
    this.prev = prev
    this.next = next
    this.list = list
...
class LinkedList extends List {
  ...
  getNodeAtIndex (index) {
    ...
    while (currentIndex < index) {
      if (currentNode.next === null) {
        throw new Error('Unexpected null at next node')
      }
  ...
  at (index) {
    index = this._parseIndex(index)
    this._wholeRangeCheck(index)

    const positiveIndex = index < 0 ? this.length + index : index
    return this.getNodeAtIndex(positiveIndex).data
  }
